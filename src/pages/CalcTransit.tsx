...
export default function CalcTransit({ navigation, route }: Props) {
  ...
  const [currLocation, setCurrLocation] = useState<Location.LocationObjectCoords | null>(null)
  ...
  const computeRoutings = function (lines: CityMapperRoute[]) {
    const routings = lines.map((route) => {
      ...
      const computedTransit: ComputedTransit = route.legs.reduce(
        (acc, curr) => {
          if (curr['travel_mode'] == 'transit' && curr['vehicle_types'].includes('bus')) {
          ...
          } else if (curr['travel_mode'] == 'transit' && curr['vehicle_types'].includes('metro')) {
      ...
      if (computedTransit.transitTags.length > 0) {
        computedTransit.transitTags = computedTransit.transitTags.flatMap((value, index, array) =>
      ...
      <View>
        ...
        {!isLoading && (
          <ScrollView>
                ...
                >
                  <View>
                    <View style={styles.transitOption}>
                      <Text style={styles.transitText}>Estimated Journey Time:</Text>
            ...
            {transits.length == 0 && (
              <Text>No routes found, You might be too close to the location already.</Text>
            )}
          </ScrollView>
        )}
      </View>
